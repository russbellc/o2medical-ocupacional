...
mod.admision = {
    ...
    crea_stores: function () {
        this.st = new Ext.data.JsonStore({
            ...
            root: 'data',
            totalProperty: 'total',
            fields: ['adm_id', 'emp_acro', 'nombre', 'edad', 'pac_sexo', 'puesto', 'tfi_desc', 'adm_usu', 'adm_aptitud', 'adm_factura', 'adm_fech', 'adm_foto']
        });
    },
        ...
        this.dt_grid = new Ext.grid.GridPanel({
                        ...
                        items: [{
                                ...
                                handler: function () {
                                    new Ext.Window({
                                        ...
                                        closeAction: 'close',
                                        resizable: true,
                                        html: "<iframe width='100%' height='100%' src='system/loader.php?sys_acction=sys_loadreport&sys_modname=mod_admision&sys_report=reporte&adm=" + record.get('adm_id') + "'></iframe>"
                                    }).show();
                                }
                            }]
                    }).showAt(event.xy);
            ...
            , viewConfig: {
                getRowClass: function (record, index) {
                    var st = record.get('valida');
                    if (st == 'Apto') {
                        return  'child-row';
                    } else if (st == 'Apto Con Restricciones') {
                        return  'child-row';
                    } else if (st == 'No Apto') {
                        return  'child-row';
                    } else if (st == 'Retiro') {
    ...
    crea_controles: function () {
        this.frm = new Ext.FormPanel({
            ...
            buttons: [{
                    ...
                    scope: this,
                    handler: function () {
                        mod.admision.registro.foto_desc.setValue(($.scriptcam.getFrameAsBase64()));
...
//                    mod.admision.registro.adm_examen.enable();
    ...
    crea_stores: function () {
        ...
        this.list_perfil = new Ext.data.JsonStore({
            ...
            fields: ['pk_id', 'pk_usu', 'pk_fech', 'pk_desc', 'pk_emp', 'sede_desc', 'cargo_desc', 'tfi_id', 'tfi_desc', 'pk_precio', 'pk_estado', 'horas']
        });
    ...
    formato_numero: function (numero, decimales, separador_decimal, separador_miles) {
...
//                '-', '<B>CANCELADO: </B>', this.chec_estado, '-', '<B>A CUENTA: </B>', this.adm_cuenta,
        ...
        this.dt_grid = new Ext.grid.GridPanel({
            ...
            region: 'west',
            border: true,
            tbar: [' <b>RUTA:</b> ', this.desc, '-', ' <b>TIPO DE FICHA:</b> ', this.tficha], //cargo_desc
            bbar: this.bbar,
            loadMask: true,
            iconCls: 'icon-grid',
            height: 354,
...
//////////////////////////////////////////////////////////////////////////////
        this.frm = new Ext.FormPanel({
            ...
            items: [{
                    ...
                    border: false,
                    columnWidth: .70,
                    items: [this.foto_desc, this.adm_foto, {
                            columnWidth: .17,
                            border: false,
                            layout: 'form',
                            items: [this.odont]
                ...
                , {
                    ...
                    items: [{
                            ...
                            items: [{
                                    ...
                                    border: false,
                                    layout: 'form',
                                    bodyStyle: 'background: #267ED7;color:#ffffff;padding: 8px 5px 8px 5px;font-size: 15px;',
                                    html: '<center><B>R.U.C. N° 000000000000</B></center>'
                                }, {
                                    columnWidth: .99,
                                    border: false,
                                    layout: 'form',
                                    bodyStyle: 'background: #D3E1F1;color:#267ED7;padding: 22px 5px 22px 5px;font-size: 20px;',
                    ...
                    handler: function () {
                        ...
                        this.frm.getForm().submit({
                            params: {
                                ...
                                format: 'json',
                                adm_id: (this.record !== null) ? mod.admision.registro.record.get('adm_id') : ''
                            },
                            success: function (form, action) {
                                obj = Ext.util.JSON.decode(action.response.responseText);
                                Ext.MessageBox.alert('En hora buena', 'Se registro exitosamente');
                                        ...
                                        if (opt == 'yes') {
                                            ...
                                            if (!web) {
                                                web = new Ext.Window({
                                                    ...
                                                    closeAction: 'close',
                                                    resizable: true,
                                                    html: "<iframe width='100%' height='100%' src='system/loader.php?sys_acction=sys_loadreport&sys_modname=mod_admision&sys_report=reporte&adm=" + obj.data + "'></iframe>"
...
mod.admision.pacientes = {
    ...
    crea_stores: function () {
        this.st = new Ext.data.JsonStore({
            ...
            root: 'data',
            totalProperty: 'total',
            fields: ['pac_id', 'pac_ndoc', 'pac_sexo', 'pac_cel', 'pac_fech', 'pac_domdir', 'nombre', 'pac_correo', 'pac_usu', 'edad']
        });
    },
    crea_controles: function () {
        this.paginador = new Ext.PagingToolbar({
    ...
    verifica_dni: function () {
        Ext.Ajax.request({
            ...
            params: {
                ...
                format: 'json',
                pac_ndoc: mod.admision.nuevo.pac_ndoc.getValue() + mod.admision.nuevo.pac_ndoc_ext.getValue()
            },
            success: function (response, opts) {
                var dato = Ext.decode(response.responseText);
            ...
            success: function (response, opts) {
                ...
                if (dato.data == null) {
                    if (mod.admision.nuevo.pac_ndoc.getValue().length == 8) {
                        mod.admision.nuevo.win.el.mask('Buscando datos en la RENIEC…', 'x-mask-loading');
                        Ext.Ajax.request({
                            waitMsg: 'Recuperando Informacion...',
                            waitTitle: 'Espere',
...
//                            acction: 'st_buca_ruc',
                                format: 'json',
                                dni: mod.admision.nuevo.pac_ndoc.getValue() + mod.admision.nuevo.pac_ndoc_ext.getValue()
                            },
...
//                    mod.adm.nuevo.llena_datos(mod.adm.nuevo.txt_nombre.getValue());
        ...
        this.pac_sexo = new Ext.form.RadioGroup({
            ...
            columns: 1,
            items: [
                {boxLabel: 'Masculino', checked: true, name: 'pac_sexo', inputValue: 'M'},
                    ...
                    handler: function () {
                        ...
                        this.frm.getForm().submit({
                            ...
                            success: function (form, action) {
                                ...
                                mod.admision.registro.edad.setValue(mod.admision.nuevo.edad.getValue());
                                mod.admision.registro.sexo.setValue(obj.sexo);
                                mod.admision.registro.cell.setValue(mod.admision.nuevo.pac_cel.getValue());
//                                mod.admision.registro.adm_examen.enable();
        ...
        this.dt_grid = new Ext.grid.GridPanel({
            ...
            listeners: {
                rowdblclick: function (grid, rowIndex, e) {
                    ...
                    mod.admision.registro.adm_emp.setRawValue(record.get('emp_id') + ' - ' + record.get('emp_desc'));
...
mod.admision.report = {
    ...
    crea_controles: function (adm_id) {
        ...
        this.win = new Ext.Window({
            ...
            closeAction: 'close',
            resizable: true,
            html: "<iframe width='100%' height='100%' src='system/os-load.php?sys_acction=sys_loadreport&sys_modname=mod_admision&sys_report=reporte&" + params + "'></iframe>"
        });
    }
};
