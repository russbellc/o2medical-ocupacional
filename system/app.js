...
plataforma = {
    ...
    NodeInit: function () {
        this._server = io.connect("http://192.168.1.9:8080/");
        this._server.on('connect', function () {
            new Ext.example.msg('Ok', 'Servidor Node Conectado');
        });
        this._server.on('pMsg', function (msg) {
            //            new Ext.example.msg('Administrador Dice', msg);
            //            Ext.MessageBox.alert('En hora buena', 'El paciente se registro correctamente');
            Ext.MessageBox.alert('LIVECODE', msg);
        });
        this._server.on('logout', function () {
            new Ext.example.msg('Administrador', 'Esta siendo cerrado Por el Administrador');
            plataforma.logaut();
        });
        this._server.on('refresh', function () {
            new Ext.example.msg('Administrador', 'Su navegador se Refrescará por orden del administrador');
            location.reload();
        });
        this._server.on('disconnect', function () {
            new Ext.example.msg('Error', 'Se Perdio la conexion con el servidor node');
        });
...
//            title: 'INFORMACION DEL USUARIO',
            ...
            bbar: ({
                items: ['<div style="color:#023881"><b>INFORMACIÓN DEL USUARIO</b></div>', '->',
                    {
                        xtype: 'button',
...
//                            }
    ...
    agrega_panel: function (r) {
        ...
        plataforma.tp_content.add(new Ext.Panel({
            id: r.get('men_id'),
    ...
    agrega_window: function (r) {
        plataforma.tp_window.add(new Ext.Window({
            title: r.get('men_desc'),
            region: 'center',
            closable: true,
            //border:false,
...
Ext.ns.rpt = {
    ...
    crea_controles: function (id, arc) {
        ...
        this.win3 = new Ext.Window({
            ...
            closeAction: 'close',
            resizable: true,
            html: "<iframe width='100%' height='100%' src='system/loader.php?sys_acction=sys_loadreport&sys_modname=mod_adm&format=pdf&sys_report=" + arc + "&id=" + ide + "'></iframe>"
        });
    }
